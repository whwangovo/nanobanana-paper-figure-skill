...
function loadCSPaperFigureTemplates() {
  const templatePath = path.join(__dirname, "..", "references", "cs-paper-figure-templates.json");
  return JSON.parse(fs.readFileSync(templatePath, "utf8"));
}
  ...
  while (parts.length > 0) {
    ...
    switch (key) {
      ...
      case "--materials-figure":
        ...
        break;
      case "--cs-paper-figure":
        args.csPaperFigure = value;
        break;
      case "--prompt-file":
...
function resolvePrompt(args) {
  ...
  if (args.materialsFigure && args.csPaperFigure) {
    throw new Error("Use either --materials-figure or --cs-paper-figure, not both.");
  }
  if (args.venue !== "generic" && !args.csPaperFigure) {
    throw new Error("--venue is only supported with --cs-paper-figure.");
  }
  ...
  if (args.csPaperFigure) {
    const subtype = loadCSPaperFigureTemplates()[args.csPaperFigure];
    if (!subtype) {
      throw new Error(`Unknown CS paper figure subtype: ${args.csPaperFigure}`);
    }
    if (!Object.prototype.hasOwnProperty.call(CS_VENUE_LABELS, args.venue)) {
      throw new Error(`Unknown venue style: ${args.venue}`);
    }
    if (!rawPrompt) {
      throw new Error("Provide paper context as the positional prompt when using --cs-paper-figure.");
    }
    let prompt = subtype[args.lang]
      .replace("{background}", rawPrompt.trim())
      .replace("{venue_label}", CS_VENUE_LABELS[args.venue]);
    if (args.styleNote) {
      prompt += `\n\nAdditional Style Requirement:\n${args.styleNote}`;
    }
    return prompt;
