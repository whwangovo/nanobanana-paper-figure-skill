...
def resolve_prompt(args: argparse.Namespace) -> str:
    ...
    if args.venue != "generic" and not args.cs_paper_figure:
        raise SystemExit("--venue is only supported with --cs-paper-figure.")
    ...
    if args.cs_paper_figure:
        if not raw_prompt:
            raise SystemExit("Provide CS paper context as the positional prompt when using --cs-paper-figure.")
        template = load_cs_paper_figure_templates()[args.cs_paper_figure][args.lang]
        prompt = template.format(background=raw_prompt.strip(), venue_label=CS_VENUE_LABELS[args.venue])
        if args.style_note:
            prompt = f"{prompt}\n\nAdditional Style Requirement:\n{args.style_note}"
        return prompt
