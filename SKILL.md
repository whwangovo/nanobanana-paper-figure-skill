...
- `scripts/build_materials_figure_prompt.py`, `scripts/build_cs_paper_figure_prompt.py`, and `--print-prompt` are local-only and do not send data over the network.
...
4. For materials-science figures or CS paper figures, prefer building the final prompt first.
   Use `python3 scripts/build_materials_figure_prompt.py --materials-figure ...` or `python3 scripts/build_cs_paper_figure_prompt.py --cs-paper-figure ...` when you want to inspect or refine the prompt before sending any API request.
...
- `scripts/build_materials_figure_prompt.py`
  Python CLI that builds materials-science figure prompts without calling the image API.
- `scripts/build_cs_paper_figure_prompt.py`
  Python CLI that builds CS paper figure prompts without calling the image API.
...
## CS Paper Figure Shortcut

If the user asks for a CS paper figure such as a method overview, pipeline, architecture diagram, mechanism figure, teaser, or conceptual comparison schematic, use the bundled CS templates instead of writing the prompt from scratch.

Workflow:

1. Read [references/cs-paper-figure-workflow.md](references/cs-paper-figure-workflow.md).
2. Pick the closest subtype:
   - `method-overview`
   - `pipeline`
   - `architecture`
...
4. Choose the output language:
   - `en`
   - `zh`
5. Insert the paper or method context into the `Project Background` slot, or use the script shortcut directly.
6. Keep the figure conceptual unless the source explicitly asks for an exact redraw.
7. If the request contains exact values, benchmark numbers, or any panel that must preserve numeric truth, switch that panel to `plot` mode instead of relying on image generation.
8. If the paper context is long, put it in a markdown file and use `--prompt-file` or `scripts/build_cs_paper_figure_prompt.py --background-file ...`.
9. For prompt refinement, consult:
   - [references/cs-paper-figure-workflow.md](references/cs-paper-figure-workflow.md)
   - [references/publication-figure-design.md](references/publication-figure-design.md)
   - [references/publication-chart-patterns.md](references/publication-chart-patterns.md)
...
python3 scripts/build_cs_paper_figure_prompt.py \
  ...
  --lang en \
  --background-file ./background.md \
  --style-note "Keep the layout clean and readable at single-column width."
```
